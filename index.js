...
class UserInfo extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const newLocal = 'SUPERAWESOME';
    // Get the template we made in our HTML and clone it so we can use it in our component
    const template = document.getElementById(newLocal).content.cloneNode(true);
    ...
    const rescript = this.getAttribute('description');
    if (rescript) {
      this.imgDescription.textContent = rescript;
    }
  }
}
